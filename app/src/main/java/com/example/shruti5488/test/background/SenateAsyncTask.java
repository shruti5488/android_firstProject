...
public class SenateAsyncTask extends AsyncTask<String, String, String> {
...
//                for (int i = 0; i < jArray.length(); i++) {
         //   Log.d("Hello", "item id " + item_id);
            if (item_id.equalsIgnoreCase("leg_senate")) {
                JSONObject jArray = new JSONObject(s);
//                        JSONObject jObject = jArray.getJSONObject(i);
                JSONArray jsonArray = jArray.getJSONArray("results");
               // Log.d("Hello", "leg senate array length " + jsonArray.length());
                for (int j = 0; j < jsonArray.length(); j++) {
                    ...
                    DataService.getInstance().setSenateList(image, name, stateName, district);
                }
            }
